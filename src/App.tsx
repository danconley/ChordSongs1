...
// Common chord set shown in the chord picker. Keep this in sync with `SONGS` suggestions in `lib/songs`.
const CHORDS: string[] = ['C', 'G', 'D', 'Em', 'Am', 'F', 'A', 'E', 'Dm'];
...
type View = 'main' | 'chords' | 'detail' | 'favorites';
...
export default function App() {
  ...
  const [discoverVisible, setDiscoverVisible] = useState<boolean>(false);
  ...
  const filteredSongs: Song[] = SONGS.filter((song: Song) =>
    selectedChords.length === 0 ? false : song.chords.every((chord: string) => selectedChords.includes(chord))
  ).slice(0, 20);
  ...
  return (
    <div className="app-bg">
        ...
        <main>
          {discoverVisible ? (
            <>
              ...
              <div className="song-list">{SONGS.map((song) => renderSongCard(song))}</div>
            </>
          ) : (
            <>
              ...
              <div className="chord-picker">
                <div className="chord-grid">
                  {CHORDS.map((chord) => (
                    <button
                      key={chord}
                      className={'chord-btn' + (selectedChords.includes(chord) ? ' selected' : '')}
                      onClick={() => toggleChord(chord)}
                      disabled={!selectedChords.includes(chord) && selectedChords.length >= 4}
                    >
                      {chord}
                    </button>
                  ))}
                ...
                <div className="chord-count">
                  {selectedChords.length === 0 ? 'Select 3–4 chords to get started!' : `${filteredSongs.length} song(s) found`}
                </div>
                <div className="song-list">
                  {selectedChords.length === 0 ? (
                    ...
                    <div className="empty-state">
                      No matches yet. Try adding or removing a chord!
                      <br />
      ...
      {view === 'detail' && activeSong && (
        <SongDetail song={activeSong} onBack={() => setView('main')} onToggleFavorite={toggleFavorite} isFavorited={isFavorite(activeSong.id)} />
      )}
